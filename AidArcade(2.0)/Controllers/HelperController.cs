...
namespace AidArcade_2._0_.Controllers
{
    ...
    public class HelperController : ControllerBase
    {
        ...
        public HelperController(AidArcadeDbContext context)
        {
            _context = context;
        }

        // Реєстрація для хелперів
        [HttpPost("register-helper")]
        public async Task<IActionResult> RegisterHelper([FromBody] RegisterDto registerDto)
        {
            if (await _context.Helpers.AnyAsync(x => x.Nickname == registerDto.Nickname))
        ...
        public async Task<IActionResult> LoginHelper([FromBody] LoginDto loginDto)
        {
            var user = await _context.Helpers.FirstOrDefaultAsync(x => x.Nickname == loginDto.Nickname);
